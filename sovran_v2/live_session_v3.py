#!/usr/bin/env python3
"""
Sovran V2 — Live Trading Session v3

Major upgrades from v2:
  - Multi-timeframe trend filter (1-min bars → macro direction)
  - Time-windowed flow (last 60s, not cumulative since start)
  - Trailing stop: activates at MFE > 1.5×SL, moves to breakeven, then trails
  - Equity consensus uses both flow AND price trend
  - Momentum regime detection (trending vs ranging)
  - Minimum conviction raised to 60 for first trade, 65 for subsequent
  - ATR-adaptive SL/TP with minimum 15 ticks
...
TRADE_COOLDOWN = 90  # seconds between trades (raised from 60)
MIN_CONVICTION_FIRST = 60    # First trade needs 60
MIN_CONVICTION_AFTER = 65    # After a loss, need 65
MIN_FLOW_TRADES = 50         # Need 50+ trades for flow reliability (raised from 30)

# Trailing stop config
TRAIL_ACTIVATION_MULT = 0.5   # Activate trail at MFE > SL_ticks * 0.5 (lowered from 1.0; 30t SL → trail at 15t)
TRAIL_BREAKEVEN_TICKS = 2     # Move SL to entry + 2 ticks (cover commission)
TRAIL_OFFSET_TICKS = 8        # Trail 8 ticks behind MFE
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("live_session_v3.log", encoding="utf-8"),
    ],
)
logger = logging.getLogger("sovran_v3")
...
def analyze_market(tick: MarketTick, meta: Dict, 
    ...
    if len(tick.bars) >= 5:
        ...
        if 5 <= atr_ticks <= 30:
            ...
            # Dynamic SL/TP
            result["sl_ticks"] = max(15, min(30, int(atr_ticks * 2.0)))
            result["tp_ticks"] = max(30, min(60, int(atr_ticks * 4.0)))
            
            # In trending regime, use wider TP (let winners run)
            if regime == "trending":
                result["tp_ticks"] = max(40, min(80, int(atr_ticks * 5.0)))
                score += 5
                signals.append("Trending TP boost")
            # In ranging, tighter TP
            elif regime == "ranging":
                result["tp_ticks"] = max(25, min(40, int(atr_ticks * 2.5)))
    ...
    # Hard block: never trade with unknown regime (safety net)
    if regime == "unknown":
        conviction = 0
        signals.append("BLOCKED: regime unknown")
...
class LiveSessionV3:
    ...
    def run(self) -> Dict:
        logger.info("=" * 60)
        logger.info("SOVRAN V3 — LIVE SESSION")
    ...
    def _check_trailing_stop(self, cid: str):
        ...
        Activates when MFE > SL_ticks × 1.5.
        First move: SL to breakeven + 2 ticks (cover commission).
        Then: trail at MFE - TRAIL_OFFSET_TICKS.
        """
        pos = self.active_positions.get(cid)
        if not pos:
            return
        
        activation_threshold = pos.sl_ticks * TRAIL_ACTIVATION_MULT
        
        if pos.mfe >= activation_threshold and not pos.trail_active:
            # Activate trailing stop — move SL to breakeven + 2 ticks
            meta = CONTRACT_META.get(pos.contract_id, {})
            ts = meta.get("tick_size", 1.0)
            ...
            # Try to modify the SL order via API
            try:
            ...
            # Already trailing — move SL up with MFE
            meta = CONTRACT_META.get(pos.contract_id, {})
            ts = meta.get("tick_size", 1.0)
            
            if pos.side == "LONG":
                ideal_sl = pos.entry_price + (pos.mfe - TRAIL_OFFSET_TICKS) * ts
            else:
                ideal_sl = pos.entry_price - (pos.mfe - TRAIL_OFFSET_TICKS) * ts
...
if __name__ == "__main__":
    ...
    session = LiveSessionV3(max_cycles=args.cycles, cycle_interval=args.interval)
    results = session.run()

    with open("live_session_v3_results.json", "w") as f:
