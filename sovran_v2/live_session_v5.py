#!/usr/bin/env python3
"""
Sovran V2 — Live Trading Session v5 (Goldilocks Edition)

All 4 Kaizen phases integrated + Goldilocks Calibration:
...
def compute_ofi_z(tick: "MarketTick", window: int = 50) -> float:
    """Order Flow Imbalance Z-Score from recent trades.
    Returns Z-score of current OFI vs rolling window.
    > 1.5 means institutional displacement above 1.5 std devs.
    """
    trades = tick.recent_trades[-window:] if len(tick.recent_trades) >= window else tick.recent_trades
    if len(trades) < 10:
        return 0.0
    ofi_values = []
    for i in range(1, len(trades)):
        buy = trades[i]["volume"] if trades[i].get("side") == "buy" else 0
        sell = trades[i]["volume"] if trades[i].get("side") == "sell" else 0
        ofi_values.append(float(buy - sell))
    if len(ofi_values) < 5:
        return 0.0
    mean = sum(ofi_values) / len(ofi_values)
    variance = sum((x - mean) ** 2 for x in ofi_values) / len(ofi_values)
    std = variance ** 0.5
    if std < 0.001:
        return 0.0
    current_ofi = ofi_values[-1]
    return (current_ofi - mean) / std


def compute_vpin(tick: "MarketTick", bucket_size: int = 50) -> float:
    """Volume-synchronized Probability of Informed Trading (VPIN).
    Buckets trades by volume and measures buy/sell imbalance per bucket.
    > 0.55 means 55%+ probability of informed order flow.
    """
    trades = tick.recent_trades[-200:] if len(tick.recent_trades) >= 200 else tick.recent_trades
    if len(trades) < 20:
    ...
    for t in trades:
        ...
        if bucket_vol >= bucket_size:
            if bucket_vol > 0:
                bucket_vpin.append(abs(bucket_buy - bucket_sell) / bucket_vol)
            bucket_vol = bucket_buy = bucket_sell = 0
    if not bucket_vpin:
        return 0.5
    return sum(bucket_vpin) / len(bucket_vpin)
...
class LiveSessionV4:
    ...
    def run(self) -> Dict:
        ...
        # V5: Seed historical bars via REST to eliminate B:0 S:0 warmup dead zone
        logger.info("Seeding historical bars via REST...")
    ...
    def _seed_historical_bars(self):
        """V5: Fetch last 30 1-min bars via REST for each contract.
        Instantly populates buy/sell volume, bars, and VWAP so the
        first scan cycle has real data instead of B:0 S:0.
        """
        ...
        for cid in SCAN_CONTRACTS:
            ...
            try:
                ...
                if not data.get("success") or not data.get("bars"):
                    logger.warning(f"  No bars for {name}: {data.get('errorMessage', 'empty')}")
                    continue

                tick = self.stream.ticks.get(cid)
                if not tick:
                    continue
                ...
                for bar in bars:
                    ...
                    tick.bars.append({"o": o, "h": h, "l": l, "c": c, "v": v, "t": 0})
                    tick.price_history.extend([o, h, l, c])
                    # Estimate buy/sell split from bar direction
                    if c >= o:  # bullish
                        tick.buy_volume += int(v * 0.6)
                        tick.sell_volume += int(v * 0.4)
                    else:       # bearish
                        tick.buy_volume += int(v * 0.4)
                        tick.sell_volume += int(v * 0.6)
                    tick._vwap_cum_pv += c * v
                    tick._vwap_cum_vol += v
                    tick.last_price = c
                    tick.high = max(tick.high, h) if tick.high > 0 else h
                    if tick.low == float("inf") or tick.low <= 0:
                        tick.low = l
                    else:
                        tick.low = min(tick.low, l)

                if tick._vwap_cum_vol > 0:
                    tick.vwap = tick._vwap_cum_pv / tick._vwap_cum_vol

                # Seed tick_count so data-sufficiency gate passes
                tick.tick_count = max(tick.tick_count, len(bars) * 4)

                logger.info(
                    f"  Seeded {name}: {len(bars)} bars | "
                    f"B:{tick.buy_volume} S:{tick.sell_volume} | "
                    f"price={tick.last_price} VWAP={tick.vwap:.2f}"
                )
    ...
    def _execute_trade(self, analysis: Dict):
        ...
        ofi_z = analysis.get("ofi_z", 0.0)
        vpin = analysis.get("vpin", 0.5)
        ...
        logger.info(f"   Goldilocks: OFI_Z={ofi_z:+.2f} | VPIN={vpin:.3f}")
