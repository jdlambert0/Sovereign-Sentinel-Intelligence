...
class WebsocketTransport(BaseTransport):
    ...
    def __init__(self, keep_alive_interval=15, **kwargs):
    ...
    def create_client(self) -> WebSocketClient:
        ...
        protocol_name = getattr(self.protocol, "protocol", "unknown")
        # FORCE binary if it's messagepack, regardless of transfer_format check
        is_bin = (protocol_name == "messagepack") or (
            getattr(self.protocol, "transfer_format", None)
            == HubProtocolEncoding.binary
        )
        # is_binary set based on protocol type
    ...
    def on_socket_close(self):
        if (
            ...
            and self.reconnection_handler is not None
    ...
    def on_message(self, app, raw_message):
        ...
        # MessagePack may decode as UTF-8 without error but produce garbage
        # Check if it's valid JSON, if not, try MessagePack unpack
        if isinstance(raw_message, bytes):
            # Try MessagePack first for binary frames
            try:
                import msgpack
            ...
            # String was passed - verify it's valid JSON, if not try MessagePack bytes
            try:
                json.loads(raw_message)
            except json.JSONDecodeError:
                # Not valid JSON - might be corrupted UTF-8 from MessagePack
                self.logger.debug(
                    "Invalid JSON received, attempting MessagePack unpack..."
                )
                try:
                    import msgpack

                    # Re-encode the string to bytes and try MessagePack
                    raw_bytes = raw_message.encode("latin-1")  # Preserve byte values
                    unpacked = msgpack.unpackb(
                        raw_bytes, raw=False, strict_map_key=False
                    )
                    raw_message = json.dumps(unpacked)
                except Exception as e:
                    self.logger.debug(f"MessagePack unpack failed: {e}")
                    # Return empty - can't parse this message
                    return []
    ...
    def send(self, message):
        ...
        try:
            self._client.send(
                ...
                opcode=0x2 if type(self.protocol) is MessagePackHubProtocol else 0x1,
            )
